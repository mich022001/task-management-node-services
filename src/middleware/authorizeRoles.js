...
const supportedRoles = new Set(['admin', 'manager', 'team_member']);

export function authorizeRoles(...allowedRoles) {
  if (allowedRoles.length === 0) {
    throw new TypeError('authorizeRoles requires at least one permitted role.');
  }

  const invalidRoles = allowedRoles.filter((role) => !supportedRoles.has(role));
  ...
  return function roleAuthorizationMiddleware(req, res, next) {
    ...
    if (!req.user.role) {
      return next(
        new AppError('The authenticated token does not contain a role claim.', {
          statusCode: 403,
